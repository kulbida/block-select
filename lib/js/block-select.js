...
(function(){
  ...
  jQuery.fn.extend({
    ...
    blockSelect: function() {
      ...
      jQuery(this).each(function (_, node) {
...
<input type="text" data-object="input" wrap="off" autocomplete="off" placeholder="Start typing a phrase" />
...
<select multiple="multiple" name="${id}" class="hidden" data-object="dest-select"></select>
          ...
          struct = jQuery(node).find("li").map(function (index, li) {
            return {
...
<a data-object="delete-tag-${id}" class="delete-tag" href="#">&times;</a>
          ...
          $dest.on("highlight-item.autocomplete", function (event, itemIndex) {
          ...
          $dest.on("go-up.autocomplete", function (event) {
          ...
          $dest.on("go-down.autocomplete", function (event) {
            if (matchItems.length) {
              autocompleteItemIndex++;
          ...
          $dest.on(`show-autocomplete-${id}`, function (event, props) {
            ...
            if (matchItems.length) {
              ...
              autocomplete.html(`<li class="nop">No items found.</li>`)
                .removeClass("hidden");
            }
            $dest.trigger("highlight-item.autocomplete", autocompleteItemIndex);
          });
...
`;
          ...
          $dest.on(`adjust-input-paddings-${id}`, function () {
            ...
            var lisWidths = $selectedTags.find("li").toArray().map(function (li) { return jQuery(li).outerWidth(); } );
            ...
            $autocompleteList.css("padding", "0");
            $autocompleteList.css("padding", "10px");
            $autocompleteList.find("li").css("padding", "10px");
            $autocompleteList.css("margin", "0");
            $autocompleteList.css("margin-top", "-5px");
            $autocompleteList.css("top", `-${parseInt($input.height)-20}px`);
            $autocompleteList.css("width", `${parseInt($input.outerWidth(true)-20)}px`);
